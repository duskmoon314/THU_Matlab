function [img_decode, ratio, PSNR, secret_decode, code] = ...
    dct_embed_3(img, QTAB, DCTAB, ACTAB, zigzag, zigzag_inv, secret)
%DCT_EMBED_3 embed secret into img
...
assert(secret_len <= blocks, "secret length needs to be smaller than pixels");
secret_pad = [secret, zeros(1, blocks - secret_len)];
...
cnt = 1;
ind = 1;
secret_decode = zeros(1, blocks);
while(cnt <= blocks)
    if(length(AC) > 11 && all(AC(1:11) == ZRL))
        ind = ind + 16;
        AC(1:11) = [];
    elseif(all(AC(1:4) == EOB))
        [~, ~, tmp] = find(AC_coeff(:, cnt));
        if(tmp(end) == 1 || tmp(end) == -1)
            secret_decode(cnt) = tmp(end);
        end
        ind = 1;
        cnt = cnt + 1;
        AC(1:4) = [];
...
secret_decode(secret_decode == -1) = 0;
secret_decode = secret_decode(1 : secret_len);
