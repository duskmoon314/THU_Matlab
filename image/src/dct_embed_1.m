function [img_decode, ratio, PSNR, secret_decode, code] = ...
    dct_embed_1(img, QTAB, DCTAB, ACTAB, zigzag, zigzag_inv, secret)
%DCT_EMBED_1 embed secret into img
...
secret_pad = [secret, zeros(1, height * width - secret_len)];
...
assert(secret_len <= 64 * blocks, "secret length needs to be smaller than pixels");

encode = reshape(bitshift(bitshift(coef, -1, 'int64'), 1, 'int64'), 1, []);
encode = reshape(encode + secret_pad, 64, []);
